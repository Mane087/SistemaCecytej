import React, { useEffect, useState } from 'react'
import loginStore from '../store/login.store';
import DataTable from 'react-data-table-component';
import { getAlumnos } from '../APi/Alumnos';
import ModalAlumno from '../Components/ModalAlumno';

const Alumnos = () => {
    const { user } = loginStore(state => ({ user: state.user, isLoggedIn: state.isLoggedIn }));
    const [alumnos, setAlumnos] = useState([])
    const [records, setRecords] = useState([])
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [update, setUpdate] = useState([]);


    useEffect(() => {
        const getAllAlumnos = async () => {
            const response = await getAlumnos()
            console.log(response);
            setAlumnos(response)
            setRecords(response)
        }
        getAllAlumnos()
    }, [])

    const handleOpenModal = () => {
        setIsModalOpen(true);  // Abre el modal

    };

    const handleCloseModal = () => {
        setIsModalOpen(false);  // Cierra el modal
        setUpdate([]);  // Limpia el estado de update
    };

 
    const columns = [
        {
            name: 'Número de Control',
            selector: row => row.numero_control,  // Usa una función para acceder a la propiedad correcta
            sortable: true,
        },
        {
            name: 'Nombre',
            selector: row => row.nombre,
            sortable: true,
        },
        {
            name: 'Apellido Paterno',
            selector: row => row.apellido_paterno,
            sortable: true,
        },
        {
            name: 'Apellido Materno',
            selector: row => row.apellido_materno,
            sortable: true,
        },
        {
            name: 'Domicilio',
            selector: row => row.domicilio,
            sortable: true,
        },
        {
            name: 'Teléfono Personal',
            selector: row => row.telefono_personal,
            sortable: true,
        },
        {
            name: 'Teléfono de Padres o Tutor',
            selector: row => row.telefono_padre_tutor,  // Asegúrate de que el nombre coincide con la API
            sortable: true,
        },
        {
            name: 'CURP',
            selector: row => row.curp,
            sortable: true,
        },
        {
            name: 'Correo Electrónico',
            selector: row => row.correo_electronico,
            sortable: true,
        },
        {
            name: 'Foto',
            selector: row => row.fotografia,
            cell: row => <img src={row.fotografia} alt={row.nombre} style={{ width: 50, height: 50 }} />,  // Muestra la imagen
            ignoreRowClick: true,
            allowOverflow: true,
            button: true,
        },
        {
            name: 'Grupo',
            selector: row => row.clave_grupo,
            sortable: true,
        },
    ];


    const handleChange = (e) => {
        const value = e.target.value.toLowerCase();
        const filteredRecords = alumnos.filter(record => 
            record.nombre.toLowerCase().includes(value) ||
            record.apellido_paterno.toLowerCase().includes(value) ||
            record.apellido_materno.toLowerCase().includes(value) ||
            record.numero_control.toLowerCase().includes(value)
        );
        setRecords(filteredRecords);
    }


    return (
        <div className='w-[90%] h-full mx-auto'>
            <div className='flex items-center py-5 gap-5'>
                <h1 className='font-bold text-2xl mr-5'>Alumnos</h1>
                {user.role === 'Administrativo' ? (
                    <>
                        <button className='w-max h-max flex items-center gap-1 text-lg border rounded-md p-2'
                            onClick={handleOpenModal}
                        >
                            <img src="data:image/svg+xml;base64," alt="icon-add" />
                            Agregar
                        </button>
                        <button className='w-max h-max flex items-center gap-1 text-lg border rounded-md p-2'
                           onClick={handleOpenModal}
                        >
                            <img src="data:image/svg+xml;base64," alt="icon-add" />
                            Actualizar
                        </button>

                    </>
                ) : null}
                <div className='w-[220px] border rounded-md p-3 flex items-center'>
                    <input
                        type="text"
                        placeholder='buscar...'
                        className='w-full'
                        onChange={handleChange}
                    />
                </div>
            </div>
            <hr />
            <div>
                <DataTable
                    columns={columns}
                    data={records}
                    selectableRows
                    pagination
                    paginationPerPage={10}
                    onSelectedRowsChange={records => setUpdate(records.selectedRows)}
                />
            </div>
            {isModalOpen && (
                <ModalAlumno
                    numero_control={update}
                    isOpen={isModalOpen}
                    onClose={handleCloseModal}
                />
            )}
        </div>
    )
}

export default Alumnos